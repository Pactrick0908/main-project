...
export class WebhookService {
   ...
   * Phân nhánh: ResaleTrade (P2P) trước, sau đó Order sơ cấp.
   */
  static async processPayOSWebhook(webhookBody: any) {
    ...
    console.log(`🔔 [Webhook] Thanh toán thành công orderCode=#${orderCode}`);
    ...
    if (p2p) {
      return {
        success: true,
        message: "Webhook processed — P2P ownership transferred, escrow HELD",
        data: p2p,
      };
    }

    // 2) Primary market order
    ...
    if (!order) {
      console.warn(`⚠️ Không tìm thấy Order/Trade với orderCode: ${orderCode}`);
